...
class ImportCredentialsCommand extends BaseCommand_1.BaseCommand {
    ...
    async initOwnerCredentialRole() {
        ...
        if (!ownerCredentialRole) {
            throw new Error(`Failed to find owner credential role. ${constants_1.UM_FIX_INSTRUCTION}`);
        }
    ...
    async getOwner() {
        ...
        const owner = ownerGlobalRole &&
            (await typedi_1.Container.get(user_repository_1.UserRepository).findOneBy({ globalRoleId: ownerGlobalRole.id }));
        if (!owner) {
