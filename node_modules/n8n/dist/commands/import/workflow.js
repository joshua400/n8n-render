...
class ImportWorkflowsCommand extends BaseCommand_1.BaseCommand {
    ...
    async initOwnerWorkflowRole() {
        ...
        if (!ownerWorkflowRole) {
            throw new Error(`Failed to find owner workflow role. ${constants_1.UM_FIX_INSTRUCTION}`);
        }
    ...
    async getOwner() {
        ...
        const owner = ownerGlobalRole &&
            (await typedi_1.Container.get(user_repository_1.UserRepository).findOneBy({ globalRoleId: ownerGlobalRole === null || ownerGlobalRole === void 0 ? void 0 : ownerGlobalRole.id }));
        if (!owner) {
            throw new Error(`Failed to find owner. ${constants_1.UM_FIX_INSTRUCTION}`);
        }
        return owner;
    }
