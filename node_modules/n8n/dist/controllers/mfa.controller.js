...
let MFAController = class MFAController {
    ...
    async getQRCode(req) {
        ...
        if (mfaEnabled)
            throw new ResponseHelper_1.BadRequestError('MFA already enabled. Disable it to generate new secret and recovery codes');
    ...
    async activateMFA(req) {
        ...
        const { decryptedSecret: secret, decryptedRecoveryCodes: recoveryCodes } = await this.mfaService.getSecretAndRecoveryCodes(id);
        if (!token)
            throw new ResponseHelper_1.BadRequestError('Token is required to enable MFA feature');
        if (mfaEnabled)
            throw new ResponseHelper_1.BadRequestError('MFA already enabled');
        if (!secret || !recoveryCodes.length) {
            throw new ResponseHelper_1.BadRequestError('Cannot enable MFA without generating secret and recovery codes');
        }
        const verified = this.mfaService.totp.verifySecret({ secret, token, window: 10 });
        if (!verified)
            throw new ResponseHelper_1.BadRequestError('MFA token expired. Close the modal and enable MFA again', 997);
    ...
    async verifyMFA(req) {
        ...
        if (!token)
            throw new ResponseHelper_1.BadRequestError('Token is required to enable MFA feature');
        if (!secret)
            throw new ResponseHelper_1.BadRequestError('No MFA secret se for this user');
        const verified = this.mfaService.totp.verifySecret({ secret, token });
        if (!verified)
            throw new ResponseHelper_1.BadRequestError('MFA secret could not be verified');
    }
};
